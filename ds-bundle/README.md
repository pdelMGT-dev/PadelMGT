...
# PadelMGTDesignSystem (padelmgt-design-system@1.0.0)

This design system is the published padelmgt-design-system React library, bundled as a single
browser global. All 23 components are the real upstream code.

## Where things are

- `_ds_bundle.js` — the whole-DS bundle at the project root; loads every component to `window.PadelMGTDesignSystem`. First line is a `/* @ds-bundle: … */` metadata header.
...
Components are then available at `window.PadelMGTDesignSystem.*`. Mount into a dedicated child node (e.g. `<div id="ds-root">`), not the host page's own React root, so the two trees don't collide:

```jsx
const { Avatar } = window.PadelMGTDesignSystem;
ReactDOM.createRoot(document.getElementById('ds-root')).render(<Avatar />);
```

## Tokens

24 CSS custom properties from padelmgt-design-system. Names are
preserved verbatim from upstream. They are declared inside `_ds_bundle.css` (this DS ships one compiled stylesheet rather than separate token files).

- **color** (2): `--text-primary`, `--text-secondary`
- **typography** (2): `--font-display`, `--font-body`
- **other** (20): `--black`, `--white`, `--neon`, …

## Components

### general
- `Avatar` — Circular avatar with photo or initials fallback. Wraps .bs-avatar from globals.css.
- `Badge` — Small uppercase pill label. Wraps the real .badge class from globals.css with a governed color palette.
- `Button` — Pill-shaped call-to-action button. Wraps the real .btn / .btn- classes from globals.css.
- `Card` — Bordered content container. Matches the border pattern reused across dashboard tables, panels and empty states.
- `CardImage` — Cropped, hover-zoom image container. Wraps the real .card-image class from globals.css.
- `Chip` — Small toggleable pill, used for filters. Wraps the real .chip class from globals.css.
- `DashboardPageHeader` — Player dashboard page header (light, eyebrow label + uppercase title). Matches Mis Ligas / Mis Juegos pattern.
- `Divider`
- `EmptyState` — Dashed placeholder box for empty lists, with title/description/CTA. Matches the Mis Ligas empty state.
- `EventPill` — Small colored calendar event label. Wraps .event-pill from globals.css.
- `Field` — Label + control wrapper. Wraps the real .field class from globals.css.
- `FieldInput`
- `FieldSelect`
- `FieldTextarea`
- `PageHeader` — Public marketing page hero header (dark, full-bleed court background). Wraps .page-header from globals.css.
- `PillTab` — Tab-style pill button, used for section/view switches. Wraps the real .pill-tab class from globals.css.
- `SectionHeader` — Marketing section header (eyebrow + big uppercase title + optional subtitle/action). Wraps .section- classes from globals.css.
- `StatTile` — Small stat block used in the player dashboard hero (value + uppercase label). Wraps .bs-tile from globals.css.
- `StatTileGrid`
- `Table` — Responsive table shell  scrolls horizontally on narrow screens. Matches the pattern used across dashboard list pages.
- `TableRow`
- `Td`
- `Th`
