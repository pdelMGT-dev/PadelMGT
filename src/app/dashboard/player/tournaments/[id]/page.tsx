        ...
        <div style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {activeRound.courts.map(court => {
              ...
              return (
                <div key={court.courtNum} style={{ background: '#fff', border: `2px solid ${isDone ? 'var(--grey-200)' : 'var(--turf-green)'}`, padding: '20px 24px' }}>
                  <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--grey-400)', marginBottom: 14 }}>Cancha {court.courtNum}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 16, marginBottom: 16 }}>
                    <div>{court.pair1.map(pid => <div key={pid} style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{getName(pid)}</div>)}</div>
                    <div style={{ textAlign: 'center' }}>
                      {isDone ? (
                        ...
                        <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--grey-300)' }}>VS</span>
                      )}
                    </div>
                    <div style={{ textAlign: 'right' }}>{court.pair2.map(pid => <div key={pid} style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{getName(pid)}</div>)}</div>
                  </div>
                  {!isDone ? (
                    <div style={{ borderTop: '1px solid var(--grey-100)', paddingTop: 14, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <input type="number" min={0} max={999} value={inputs.p1} onChange={e => setScoreInputs(prev => ({ ...prev, [key]: { ...(prev[key] ?? { p1: '', p2: '' }), p1: e.target.value } }))} placeholder="0" style={inp} />
                      <span style={{ color: 'var(--grey-400)', fontWeight: 700 }}>–</span>
                      <input type="number" min={0} max={999} value={inputs.p2} onChange={e => setScoreInputs(prev => ({ ...prev, [key]: { ...(prev[key] ?? { p1: '', p2: '' }), p2: e.target.value } }))} placeholder="0" style={inp} />
                      <button onClick={() => handleRegister(activeRound.num, court.courtNum)} style={{ padding: '9px 20px', background: 'var(--black)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                        Registrar
                      </button>
                    </div>
                  ) : (
                    <div style={{ borderTop: '1px solid var(--grey-100)', paddingTop: 10, fontSize: 11, color: 'var(--turf-green)', fontWeight: 600 }}>✓ Score registrado</div>
                  )}
                </div>
              );
            })}
