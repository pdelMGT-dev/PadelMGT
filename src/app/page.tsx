          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 'clamp(40px, 6vw, 80px)', alignItems: 'center' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', padding: '32px 40px', minWidth: 280, flexShrink: 0 }}>
              ...
              {(pzTiers.length ? pzTiers : [
                ...
                <div key={tier.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 0', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none', gap: 24 }}>
                  <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.72)', whiteSpace: 'nowrap' }}>{tierLabelInList(tier, arr)}</span>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 600, color: '#fff' }}>${tier.price}</span>
                </div>
              ))}
              {pzPromo?.displayText && (
                <div style={{ marginTop: 14, fontSize: 12, color: 'var(--neon)', textAlign: 'center', fontWeight: 600 }}>{pzPromo.displayText}</div>
              )}
